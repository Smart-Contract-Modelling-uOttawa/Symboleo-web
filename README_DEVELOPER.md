...
Symboleo-web primarily uses Monaco-Editor, developed by Microsoft. The editor supports a myriad of programming languages, boasts features such as syntax highlighting, autocompletion, and error diagnostics, and is renowned for its smooth and responsive user interface. This is combined with LSP4J, a Java implementation of the Language Server Protocol, to be used by Language Servers built using Java(like XText and Xtend) to ensure the editor adheres to the Language Server Protocol. The Language Server Protocol (LSP) defines the protocol used between an editor or IDE and a language server. The LSP standardises the communication by different client thus enabling reusability of Language Servers. By adhering to the LSP, the server can be used by any client (VSCode desktop application, Web IDE etc.). Websockets are used to facilitate communication between the client and the language server. The architecture is shown in below

![image](https://github.com/Smart-Contract-Modelling-uOttawa/Symboleo-web/assets/41239586/ca82feba-d2ba-43a3-b1bc-9351faab31f4)

A client, opens up a new instance using gitpod. A request is made to the backend to initiate a new instance. Symboleo uses Websockets to achieve this. This is done to maintain a lasting connection with the server. This is done so that we don’t have to send the entire document for diagnosis for a simple change but only the new changes are sent to server to optimise performance. Once this instance is created and a connection is established with the server, we request diagnostics on the current state of the document. All these messages can be observed in the network tab in browser developer tools. A screenshot of this is attached below. 

![image](https://github.com/Smart-Contract-Modelling-uOttawa/Symboleo-web/assets/41239586/ee43f6aa-e3d2-4c54-a8ce-d47affbb910f)
...
1. [Monarch Language(for syntax highlight)](https://microsoft.github.io/monaco-editor/monarch.html)
...
5. [Docker(for autorun everytime gitpod is launched)](https://www.gitpod.io/docs/configure/workspaces/tasks)
...
1. [ElkGraph Web(Similar Project)](https://github.com/kieler/elk-live)
